...
if (bwin) {
  ...
  if (reduceMotion) {
    ...
    replyEl.innerHTML = replyState('bw-ok', ID_REAL);
    replyEl.classList.add('show');
    note.classList.add('show');
    cap.textContent = 'masked before send, answered, restored in place';
  } else {
    ...
    const run = () => {
      ...
      later(() => {
        userEl.innerHTML = userState('bwin-mask', ID_MASK);
        note.classList.add('show');
        cap.textContent = 'masked before send. the page never transmits the real ID';
      }, typed + 2700);
      later(() => {
        replyEl.innerHTML = replyState('bwin-mask', ID_MASK);
        replyEl.classList.add('show');
        cap.textContent = 'claude answers, seeing only the stand-in';
      }, typed + 4700);
      later(() => {
        replyEl.innerHTML = replyState('bw-ok', ID_REAL);
        cap.textContent = 'restored in place. the analyst never noticed';
      }, typed + 6700);
      later(() => {
        cap.textContent = 'evidence written: browser event, masked (1 span)';
      }, typed + 8700);
      later(run, typed + 11500);
    };
    run();
  }
}
